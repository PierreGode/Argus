#!/bin/bash
# macOS installer for Argus daemon (Python + bleak + launchd).
# Mirrors install.sh but uses LaunchAgents instead of systemd user units.
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SERVICE_LABEL="com.user.argus-daemon"
...
DAEMON_PY="$SCRIPT_DIR/daemon/argus_daemon.py"
LOG_DIR="$HOME/Library/Logs"
LOG_OUT="$LOG_DIR/argus-daemon.out.log"
LOG_ERR="$LOG_DIR/argus-daemon.err.log"

echo "=== Argus macOS install ==="
echo ""
...
echo "  3. Click 'Connect' next to 'Argus Controller'."
echo "  4. The daemon will discover it within ~30 s and start polling."
echo ""
echo "Useful commands:"
echo "  launchctl list | grep argus            # check it's running"
