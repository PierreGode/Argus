...
void ble_init(void) {
    ...
    // 250 chars since we added the Copilot + apps fields. NimBLE advertises
    // this as the device's preferred MTU during the ATT_MTU exchange the
    // central kicks off right after connect; pick the largest value the spec
    // allows so even hex-heavy future fields fit one write.
