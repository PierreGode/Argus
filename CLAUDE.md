...
- PMU: **AXP2101** on same I2C bus (addr=0x34) — battery, USB VBUS, PWR button IRQ. **LCD backlight is on the AXP2101 `DLDO1` rail** (not a GPIO — the `expander` pin-6 toggle in `main.cpp`'s reset sequence is vestigial); power-save blanks it via `power_set_backlight()` → `pmu.enable/disableDLDO1()`. `power.cpp` is a real XPowersLib driver (battery %, charging, backlight); needs `lewisxhe/XPowersLib`.
...
**The wire payload** is built in `build_payload()` and parsed in `main.cpp`'s `parse_payload()`. Keys are short to fit the BLE MTU — `s`/`sr`/`w`/`wr` (caps), `c`/`cw`/`mo`/`ms`/`mh`/`mf`/`ch`/`tk`/`se`/`pj` (today — `mo`/`ms`/`mh`/`mf` are Opus/Sonnet/Haiku/Fable token share; all local-machine-only, unlike the `s`/`w` rate-limit caps above which are account-wide), `ge`/`gi`/`gp` (GitHub), `cie`/`cis`/`cir`/`cib`/`ciw`/`cif`/`ciq` (CI/CD — GitHub Actions, via `ci_stats.py`), `cp`/`cps`/`cpw`/`cpe`/`cpr`/`cpp`/`cpu`/`cpa`/`cpsu`/`cpm` (Copilot), `apps` (enabled-screens CSV), `tac`/`tch` (Today screen: show cost panel / cache panel), `md`/`evts` (splash mood + events strip), `br` (brightness), `fc` (one-shot auto-focus), `nm` (BLE device-name override — firmware persists it to NVS and re-advertises, so multiple Argus units don't all collide on `"Argus Controller"`), `ps`/`pst`/`chg` (power-save enable / poll-aware idle timeout in seconds / substantive-change wake flag — firmware blanks the LCD backlight via the AXP2101 **DLDO1** rail after `pst` idle, waking on `chg`/touch/button). The firmware ignores unknown keys, so the two sides can version independently. The full table lives in README.md.
